...
from link_parser import process_message_text
...
class BatchBuffer:
    ...
    async def _worker(self) -> None:
        ...
        while True:
            try:
                # Wait for either timeout or explicit flush signal
    ...
    async def _force_flush(self) -> None:
        ...
        try:
            # Enrich + dedupe + build DB records
            records = []
            for item in items:
                # Ensure user exists
                ...
                # Enrich URL data (titles, platform, etc.)
                enriched = await process_message_text(session, item.url_data["url"])
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Quick check: does text contain URLs?
    from link_parser import extract_urls
    ...
    # Optional: react with emoji so user knows bot saw the link
    try:
        await update.effective_message.set_reaction("👀")
    except Exception:
        pass  # bot may not have reaction perms
