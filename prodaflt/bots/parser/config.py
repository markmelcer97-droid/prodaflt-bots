...
# Load .env from project root (3 levels up: parser/ → bots/ → prodaflt/ → root)
...
class Config:
    ...
    _admin_raw = os.getenv("ADMIN_IDS", os.getenv("ADMIN_CHAT_ID", ""))
    ADMIN_IDS: List[int] = [
        int(x.strip())
    ...
    # Batch settings
    BATCH_SIZE: int = int(os.getenv("PARSER_BATCH_SIZE", os.getenv("BATCH_SIZE", "10")))
    BATCH_TIMEOUT_SECONDS: int = int(os.getenv("PARSER_BATCH_TIMEOUT", os.getenv("BATCH_TIMEOUT_SECONDS", "30")))
    ADMIN_IDS: List[int] = [
        int(x.strip())
